...
while (1):
    ...
    for row in rows3:
        ...
        if (len(rowsy) > 0):
            ...
            r = requests.get("http://127.0.0.1:7777/flash?job=yes&find="+r_json["search"].split("?q=")[1]+"&percent="+str(r_json["percent"])+"&host="+r_json["search"].split("/catalog/?q=")[0])
            ri = r.json()
